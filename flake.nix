    ...
    in
    {
      ...
      checks = forSystems (system:
        ...
        in
        {
          ...
          validate-modular-aggregate-export = pkgs.runCommand "validate-modular-aggregate-export" {
          ...
          in pkgs.runCommand "fleetix-lib-helpers" {} ''
            test "${self.lib.hosts.resolveHostAddress { inherit topology; hostName = "atlas"; policy = [ "lan" ]; }}" = "192.168.178.88"
            test "${self.lib.hosts.resolveHostAddress { inherit topology; hostName = "nomad"; policy = [ "direct-link" ]; }}" = "10.10.0.2"
            test "${endpoint.url}" = "http://10.10.0.1:2283"
            test "${if missingEndpoint == null then "null" else "unexpected"}" = "null"
            test "${self.lib.domains.zoneForHost { inherit topology; fqdn = "host.internal.example.test"; }}" = "internal.example.test"
            test "${self.lib.domains.relativeName { fqdn = "wg.example.test"; zone = "example.test"; }}" = "wg"
            ...
            test "${toString (self.lib.firewall.lanExposedPorts { inherit topology; hostName = "atlas"; })}" = "2283"
